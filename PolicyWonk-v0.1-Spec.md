...
> **Pivot — 2026-06-11. The configuration surface is no longer a seven-screen onboarding wizard.** Configuration moves to a GitHub-style `/settings/` page with five panels (GitHub App, AI provider, Policy repository, Diocese configuration, Users-and-roles); inventory becomes a top-level `/inventory/` page with an always-present drop bucket; first-boot UX is a seeded `admin`/`admin1234` account that forces a strong-password change on first login and lands on the catalog with a tiered "configure your environment" banner pointing at Settings; GitHub App provisioning automates via the **GitHub App manifest flow** with Installation ID auto-detect and repo dropdown so the admin clicks three buttons instead of copy-pasting App ID, Installation ID, and a PEM. The authoritative replacement spec is at `internal/superpowers/specs/2026-06-11-settings-page-design.md`. **Sections of this PRD that still hold post-pivot:** Problem Statement, Architecture in Brief, Frontend Portability Constraints, Licensing, Goals, Non-goals, the policy-data and gate-and-review behaviors (P0.1-P0.6, P0.X foundational reference docs), the handbook generation behavior (P0.7) sans the wizard-publishing references, the Engineering Lanes, the Tech section, and most v0.2 deferrals. **Sections that the new spec supersedes:** the seven-configuration-screen text in P0.8 onboarding, the APP-29 wizard-completion-screen behavior (replaced by the catalog banner + the Settings page's per-panel setup actions), the "seven-screen wizard" framing in OQ-13's UI-framework resolution, the P2.11 "wizard step 6" anchor, the P2.7 "wizard-managed handbook publishing" v0.2 trajectory (which now extends the Settings Policy-repository panel, not a wizard), and the Week-4 / Week-5 weekly progress notes that frame wizard screens as in-scope. The wizard-related architectural commitments still anchor diocesan UX (seven knobs of configurable per-diocese choice; address scheme, versioning, reviewer roles, retention defaults, LLM provider, policy repo, foundational reference docs); they live as Settings panels now rather than as a linear wizard.
...
Acceptance:
- Given a catalog of 50+ ingested files, when the inventory pass runs, then a corresponding markdown file with complete YAML front matter is staged for commit, with confidence scores and per-call usage telemetry (provider name, model id, input and output token counts, call timestamp) recorded in a separate `.audit.yaml` file (confidence per AI-07, usage per AI-16). The usage block supports cost-attribution debugging in v0.1 and is the data foundation for v0.2 spend ceilings (P2.11).
...
**P2.8 AI-assisted policy authoring.** Architectural note: extend the existing AI provider abstraction to support conversational drafting flows alongside the v0.1 extraction flow. A new "Draft a policy" entry point on the catalog opens an HTMX-driven chat surface where the CFO describes the policy in plain language and the AI proposes a complete markdown draft with frontmatter pre-filled. New-policy creation reuses the PR-as-gate pattern unchanged (Drafted = open PR). The foundational bundle plus existing published policies in the same classification supply grounding context. Every factual claim the AI generates (regulatory citations, IRS publication numbers, USCCB references) carries a "needs verification" badge that the reviewer must clear before approval. Brainstorm: `internal/PolicyCodex-v0.2-Brainstorm.md`.

**P2.9 Targeted policy re-extraction.** Architectural note: every drafted or published policy carries a back-reference to the source manifest entry it was extracted from (already present per INGEST-04). v0.2 adds a "Re-extract from source" affordance on the policy detail view that runs extraction against the single source file, opens a PR showing the diff, and lets the normal Drafted/Reviewed/Published gates handle the change. Blast radius is one policy. Common triggers: source document updated outside PolicyCodex, original AI suggestion was poor, foundational policy changed and one classification needs re-derivation. Brainstorm: `internal/PolicyCodex-v0.2-Brainstorm.md`.

**P2.10 Bulk corpus re-ingest with gate-state preservation.** Architectural note: the v0.1 inventory pass is re-run safe (existing slugs are skipped, never clobbered). v0.2 adds three explicit re-ingest modes: (a) re-extract only Drafted policies (default, no human work lost); (b) re-extract Drafted plus human-edited drafts but not Reviewed or Published policies (confirmation required); (c) archive everything to `policies/.archive/<timestamp>/` and start over (power-user CLI only, recoverable via Git history). Every re-ingest writes an audit entry naming who triggered it, when, what got replaced, and what was preserved. Brainstorm: `internal/PolicyCodex-v0.2-Brainstorm.md`.

**P2.11 AI provider key management, cost controls, and audit logging.** Architectural note: every AI call already writes to a confidence-audit sidecar (`.audit.yaml`) in v0.1. v0.2 layers a monthly spend ceiling collected during wizard step 6 on top; PolicyCodex refuses to call once the budget is exhausted and emits a clear error. **Two pieces of the v0.2 minimum were pulled forward to v0.1 in the 2026-06-08 brainstorm pass and filed as polish-week tickets: APP-31 (wizard step 6 prose + README install section + expected-cost table) sets diocesan expectations correctly at install time, and AI-16 (audit-sidecar extension with provider, model, token counts, and timestamp) gives diocesan IT directors the cost-attribution data they need if API spend looks off.** The diocese supplies API credentials, not consumer subscriptions: Anthropic API key (not Claude Pro / Max / Teams), OpenAI API key (not ChatGPT Plus), Gemini API key or Vertex AI credentials (not Google One), Azure OpenAI deployment credentials, or a local Llama endpoint requiring no third-party key at all. The wizard step 6 prose and the public README must state this distinction explicitly so diocesan IT directors do not arrive expecting their existing claude.ai subscription to work. Brainstorm: `internal/PolicyCodex-v0.2-Brainstorm.md`.

**P2.12 Document owner policy upload with overlap detection.** Architectural note: extend the per-policy extraction (`ai/inventory_extract.py`) with a non-bulk entry point and an "Add policy" UI surface so the CFO / HR Director / Document Control Owner personas have a flow that does not require waiting for the next bulk inventory pass. New uploads check for semantic overlap against existing policies via the embedding index introduced for P2.1 (Q&A chatbot RAG over the handbook). Same index, two payoffs. When overlap is detected, the document owner sees three explicit choices with AI-recommended defaults: merge into the existing policy (preserves slug and address), replace it (`deprecated: true` on the old, new slug for the new, mutual references on both), or create as parallel (AI was wrong about overlap). The resolution emits a PR through the normal Drafted/Reviewed/Published gate. Address strings stay stable through resolution because handbook URLs are stable. Whole-taxonomy renumbering is out of scope and reserved for v0.3+ if at all. Brainstorm: `internal/PolicyCodex-v0.2-Brainstorm.md` section 6.
