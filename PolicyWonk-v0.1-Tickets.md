...
| APP-26 | Post-DISC: foundational editor hardening + demo polish. Carved from APP-25 (2026-06-05), which shipped the demo slice. Scope: (1) title-case the typed-table column headers + add per-field labels (currently raw schema keys — reads as raw-schema on a projector); (2) client-side multi-row add (today `extra=1` adds one retention row per save round-trip); (3) pagination/virtualization for a full ~150–240-row schedule rendered on one page; (4) soft-delete (`deprecated: true`) semantics for a classification still referenced by other policies (per the foundational-policy design); (5) optimistic-concurrency / mtime guard on `data.yaml` (shares `policy_edit`'s single-admin no-reload assumption — also leaves a dirty working copy if the PR open fails after the local write); (6) reuse the editor from the APP-23 detail view. | L | post-DISC | APP-25, APP-23 |
