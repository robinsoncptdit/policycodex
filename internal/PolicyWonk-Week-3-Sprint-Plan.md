...
| APP-06 Catalog list view | subagent | 2 days | APP-01 (done), APP-05 | **DONE 2026-05-15** (merge `a681d0f`). Function-based `catalog` view with `@login_required`, reads from local working copy, renders foundational bundles + flat policies in one list with kind badges and `(foundational)` marker. Establishes `core/templates/base.html` as project's first base layout; `/` redirects to `/catalog/`. 3 commits, 9 new tests. Reviewer APPROVE; 2 Important nits noted as follow-ups (semantic `<nav>` scoping, `or`→`and` assertion). Gates APP-07, APP-20. |
| APP-21 L3 startup self-check | subagent | 1 day | APP-05, INGEST-07 | **DONE 2026-05-15** (merge `9aabce8`). Django system-check via `@register()` in `app/working_copy/checks.py`, wired through new `WorkingCopyAppConfig`. New `POLICYCODEX_ONBOARDING_COMPLETE` setting gates severity (W001/W002 onboarding-mode infra failures vs E004/E005 post-onboarding); bundle-validity always Error (E001/E002/E003). 6 commits (5 plan + 1 review-fix), 15 new tests. Reviewer APPROVE WITH FIXES applied in-branch (Path import load-bearing, `manage.py check` assertion strengthened). |
| APP-07 Edit form opens PR | subagent | 2 days | APP-04 (done), APP-06 | **DONE 2026-05-16** (merge `1a8e626`). Function-based `policy_edit` view at `/policies/<slug>/edit/` with `@login_required`. Form fields: title + body + summary (other frontmatter preserved lossless via `_render_policy_md` helper). Foundational gate returns HTTP 403 with custom template. Branch naming `policycodex/edit-<slug>-<uuid>`. 7 commits (6 plan + 1 review-fix), 21 new tests. Reviewer APPROVE WITH FIXES (empty-frontmatter round-trip + TOCTOU note). |
| APP-17 PR-state-to-gate mapping | subagent | 1 day | APP-04 (done), APP-07 | **DONE 2026-05-16** (merge `02739b3`). `GitHubProvider.list_open_prs` batches one `repo.get_pulls(state="open")` call returning `list[dict]` with pr_number/head_branch/gate/url. `_pr_to_gate` refactored into module-level helper at `app/git_provider/states.py` shared with `read_pr_state`. Branch-to-slug parser supports both `policycodex/draft-` and `policycodex/edit-<slug>-<hex>` conventions. Catalog template gains gate-badge per row; graceful degradation to "Published" on provider failure. 5 commits, 21 new tests. Reviewer APPROVE WITH FIXES (narrowed exception). Dispatch wrinkle: implementer accidentally committed to parent main; rolled back via `git update-ref` with Chuck's authorization. |
| APP-18 Approve action | subagent | 1 day | APP-04 (done) | **DONE 2026-05-16** (merge `3674997`). `GitHubProvider.approve_pr` calls PyGithub `create_review(event="APPROVE")`. View at `/policies/approve/` gate-guards on `read_pr_state == "drafted"`. Catalog form posts a PR number (per-row buttons land with APP-17 + APP-19 integration). Permission model v0.1: any authenticated user. 6 commits (5 plan + 1 review-fix), 21 new tests. Reviewer APPROVE WITH FIXES (named URL + positive pr_number guard). |
| APP-19 Publish action | subagent | 1 day | APP-04 (done), APP-17 | **DONE 2026-05-16** (merge `0125323`). `GitHubProvider.merge_pr(merge_method="squash")` wraps PyGithub merge. View at `/policies/<slug>/publish/` locates PR via `list_open_prs` + `branch_to_slug` (no sidecar; pivoted from plan's policymeta after review found APP-07 doesn't produce sidecar). Gate-guards on `read_pr_state == "reviewed"`. Catalog template renders Publish button only on Reviewed rows. 6 commits (5 plan + 1 review-fix), 21 new tests. Reviewer APPROVE WITH FIXES (sidecar→list_open_prs pivot + audit log). **End-to-end edit-flow demo-ready** (edit → Drafted → Approve → Reviewed → Publish → Published). |
