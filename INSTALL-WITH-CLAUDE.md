...
2. Create `~/.config/policycodex/config.env`. This is where the GitHub App credentials and the LLM API key go. Tell me which values it needs by reading `HOWTO-GitHub-Team-Setup.md` and `app/git_provider/github_config.py` in the repo (so you give me the current, exact key names rather than guessing). At minimum it includes `POLICYCODEX_GH_PRIVATE_KEY_PATH`, which must point at a path **inside** `/secrets/` (for example `/secrets/github-app-private-key.pem`), because the host file at `~/.config/policycodex/github-app-private-key.pem` appears at `/secrets/...` inside the container.
...
This prompt mirrors the live Docker install path. **Update it in the same change whenever any of these moves:** `Dockerfile`, `docker-compose.yml`, `docker-compose.pull.yml`, `docker/entrypoint.sh`, `install.sh`, `.env.example`, the credentials convention (`~/.config/policycodex/`, `POLICYCODEX_CONFIG_PATH`, `app/git_provider/github_config.py`), the onboarding/login URL routing (`policycodex_site/urls.py`, `core/urls.py`, `app/onboarding/urls.py`), or the inventory-pass command (`core/management/commands/run_inventory_pass.py`). A lockstep guard, `tests/test_install_prompt_sync.py`, fails the suite if this prompt is left pointing at a file, env key, install command, or URL route that no longer exists — so drift in the tracked files trips a red test, not just a stale doc. It cannot check that the prose is *accurate*; a human still reviews wording. Verified against the codebase 2026-06-08 (Python 3.14 / Django 6.0, REPO-05 + REPO-11 + REPO-15).
