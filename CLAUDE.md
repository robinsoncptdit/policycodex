...
**Settings-page rebuild SHIPPED (2026-06-11) + polished (2026-06-12); REPO-10 audit run and all 14 findings resolved (2026-06-14).** The 2026-06-11 pivot decision (tear down the seven-screen onboarding wizard, replace it with a Settings page) was executed the same evening: all six phases / 30 tasks of the rebuild plan landed on `main` as feature+cleanup commit pairs, followed by a 13-commit UX polish sprint (`ux-fix-1`..`ux-fix-13`) the next morning (`978a50a`). On 2026-06-14 the full REPO-10 generic-ship + 7-track audit ran (ship-generic gate **PASS**, zero PT leakage; 14 findings F1–F14) and **every finding was fixed** across three subagent-driven batches. **Current code tip `6924995`**; suite green at **710 passed, 11 skipped, 0 failed** (run it with `ai/venv/bin/python -m pytest`). All three audit-fix batches are pushed to public `origin/main` (synced through the `3bd6bf1` doc commit, 2026-06-14).

**Problem #2 fixed — inventory drop bucket now uploads (2026-06-15, commits `734517b..fa382dd`, LOCAL `main`, NOT yet pushed — network was down at close).** Drag-and-drop opened the PDF in Chrome and "Add policies"/picker did nothing. Reproduced first with Interceptor, root-caused as purely client-side: the bucket was a bare `<label>` + hidden `<input type=file>` with a `hidden` submit button, no `change` handler, no drop handler (only htmx loaded), and "Add policies" was a scroll-only anchor; the `inventory_upload` endpoint already worked but never got POSTed. Fix = new `static/js/inventory-upload.js` (window-level drag/drop `preventDefault`; dropzone drop→assign files→submit; input `change`→submit; "Add policies"→open picker) + DOM hooks (`bucket-dropzone`/`bucket-input`/`add-policies`) + a reusable `{% block extra_scripts %}` in `base.html` + a template-wiring test; the DISC smoke was de-masked (it had been hiding the bug with a forced `requestSubmit()`). Built via subagent-driven-development under ultracode; **subagent Edit was permission-denied this session so the controller implemented directly with read-only review subagents + a final 4-lens adversarial Workflow** (10 findings, all minor; 4 applied incl. a double-submit re-entrancy guard, the rest documented as deliberate/out-of-scope). **Verified live in real Chrome via Interceptor** on a working-tree dev server (`:8011`; the `:8000` container runs the old image): change→run created, synthetic drop→`defaultPrevented:true`+file assigned+run created. Suite **710 → 711 passed, 11 skipped, 0 failed**. Detail in `internal/PolicyWonk-Daily-Log.md` (2026-06-15 night) and plan `internal/superpowers/plans/2026-06-15-inventory-upload-wiring.md`. **Only open step: `git push origin main` once network returns.**
...
**GitHub App auth unified on PyGithub + clock-drift errors (2026-06-15, commit `a714c98`, public `main`):** prompted by reviewing the connect flow against GitHub's JWT doc for seamless self-hosted deploy. The manifest install-check's `_list_installations` was the only hand-rolled JWT (pyjwt); replaced with `app/git_provider/github_provider.py` `list_app_installations()` using PyGithub's `GithubIntegration.get_installations()`, so all GitHub App auth + JWT generation now lives in one place (`_list_installations` is a thin delegation). Added `friendly_github_auth_error()` mapping GitHub's exp/iat 401 time-claim text to a "sync the server clock / enable NTP" hint, wired into the install check and `GitHubProvider.test_credentials` — clock drift is the most likely confusing failure on a self-hosted diocese VM. `HOWTO-GitHub-Team-Setup.md` gained an NTP prerequisite bullet. (`iss` still uses App ID, which GitHub keeps backward-compatible though it now recommends client ID — deliberately left as-is.)

**Working-copy concurrency lock (2026-06-15, commit `6c35aec`, public `main`):** second instance of the "single-process code under gunicorn `--workers 3`" bug class. `app/git_provider/propose.py` `propose_change` mutated one shared working copy with no lock; concurrent saves raced on the same `.git` ("pathspec did not match any files" on the diocese-configuration save). Added `working_copy_lock(working_dir)` (`fcntl.flock`) wrapping the whole write+propose critical section at all four call sites (config save, policy edit, foundational edit, inventory finalize); locking only `propose_change` is insufficient and the lock is not taken inside it (self-deadlock). Verified in the rebuilt `~/pcx-test` container. The recurring pattern is captured in memory `project_shared_state_across_workers.md` — **when adding worker-touched code, check shared mutable state (module caches need cross-process invalidation; shared on-disk mutations need a cross-process lock).**

**Retention-policy upload restored (2026-06-15, commits `5d2790b..6924995`, public `main`):** the 2026-06-11 pivot tore out wizard screen 7 (APP-15, the source-of-truth retention upload) and never reimplemented it, leaving NO UI to create the `policies/document-retention/` foundational bundle that the inventory pass hard-requires (`load_foundational_taxonomy` → `InventoryNotReady`). Rebuilt as **Settings → Policy repository → "Upload retention policy"**: a new `app/settings/retention_setup.py` `scaffold_retention_bundle()` wires the previously-orphaned pieces — `ingest.extractors.extract` (PDF/DOCX/MD/TXT → text), `ai.retention_extract.extract_retention_bundle` + `build_data_yaml` (text → validated `data.yaml`), and `propose_change` — writing `policy.md` (`foundational: true`, `provides: [classifications, retention-schedule]`) + `data.yaml` and opening one PR, all under `working_copy_lock`. New `upload_retention` action + `SetupAction` on `PolicyRepoPanel`, multipart upload form (gated on a configured repo), and the inventory not-ready message now points users at the action. Built via subagent-driven-development under a workflow (4 tasks, two-stage review each + final adversarial review; the review gate correctly caught a defect in the plan's own test before any bad code shipped). Verified live in the rebuilt `~/pcx-test` container (panel renders the upload affordance; LLM + repo + App all configured). The companion **foundational editor** (`/policies/<slug>/foundational-edit/`) still handles manual edits once the bundle exists. Out of scope: OCR for scanned-image PDFs (surfaced as a clear error), and a manual "USCCB starter schedule" create path.
