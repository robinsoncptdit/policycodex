...
**Week-3 Wave-1 status (2026-05-14, 4 days ahead of plan):** all 4 Wave-1 Committed tickets merged: APP-05 (`e9a41cc`), INGEST-07 (`7035946`), PUBLISH-01 (`7427ffc`), AI-06 (`282b2d7`). Plus OQ-12 resolved (`ff3ac47`) and the PT bundle scaffold landed on `pt-policy@34a1671`. Code review ran on every branch before merge; INGEST-07 + AI-06 each had Important review fixes applied in-branch before merge. 156 tests passing on main (was 116 going in). Astro 5.18.1 builds 4 pages clean. **Subagent dispatch lesson:** `skipAutoPermissionPrompt: true` + no project-level allow rules silently denied subagent Bash to `/tmp/pc-wt-*` worktrees and Writes to `internal/superpowers/plans/`; pivoted to in-session execution for Wave-1. Added `Write(internal/superpowers/plans/**)` and `Write(internal/superpowers/specs/**)` to project allow list. Future worktrees should land inside the project tree (e.g., `.worktrees/<ticket>/`) to keep subagent Bash unblocked. Wave-2 (APP-06 catalog view + APP-21 startup self-check) unblocked.
