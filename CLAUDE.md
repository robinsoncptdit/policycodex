...
- `repo-template/` holds generic, vendorable files copied into a diocese's policy repo during onboarding: `.github/workflows/foundational-guard.yml` plus `.github/scripts/foundational_guard.py` (the L2 pre-merge CI guard that blocks deleting a `foundational: true` file or emptying a `provides:` list; REPO-09), `.github/workflows/build-handbook.yml` plus the vendored `handbook/` Astro project and `sync-handbook.sh` re-vendor script (the merge-to-handbook build; PUBLISH-06), and a README. Its `repo-template/tests/` run in the main suite but are not copied on install. This is the canonical home for installable diocese-repo automation
- `ai/` holds the LLM provider abstraction (`provider.py`), Claude implementation (`claude_provider.py`), `emit.py` (markdown + YAML front-matter emitter), `audit.py` (the inverse of `emit.py`: emits a `<slug>.audit.yaml` confidence sidecar; AI-07), `taxonomy_loader.py` (Django-free loader that finds a diocese's foundational-policy bundle by capability and reads its `data.yaml`, with seed fallback; AI-12), `gap_detection.py` (Django-free: flags policies whose `category` is not in the bundle classifications; AI-13), `taxonomies/` (per-diocese classification + retention seeds; `pt_classification.yaml` is install-zero's), and `ai/tests/`
- `app/` holds App-lane code: `app/git_provider/` (`GitProvider` ABC + `GitHubProvider` with clone, branch, commit, push, open-PR, read-PR-state), `app/working_copy/` (local working-copy config + manager + the L3 startup self-check), `app/onboarding/` (the seven-screen onboarding wizard: step registry, session-backed `WizardState`, routing and navigation from the APP-08 skeleton, plus `forms.py` = a slug->form registry the generic step view binds/validates/persists, with screen 1's `GitHubRepoForm` landed in APP-09; remaining per-screen content is APP-10..16), and `app/requirements.txt`
- `ingest/` holds the Ingest-lane code: `local_folder.py` (`LocalFolderConnector` with the `python -m ingest.local_folder <path>` CLI), `extractors/` (PDF via pypdf, DOCX via python-docx, MD/TXT via `read_text`; dispatched by extension via `from ingest.extractors import extract`), `policy_reader.py` (`BundleAwarePolicyReader` + `LogicalPolicy`: reads flat `<slug>.md` and `<slug>/policy.md` bundles as one inventory; INGEST-07), `manifest.py` (source manifest data model: frozen `ManifestEntry` + SHA-256 content hash, the INGEST-05 change-detection key; INGEST-04), and `ingest/tests/`
